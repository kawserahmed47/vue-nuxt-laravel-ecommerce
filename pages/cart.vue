<template>
  <div class="container py-20">
    ...
    <table class="table-auto w-full">
      ...
      <tr>
        <td class="border px-4 py-2 text-center " colspan="5">Total</td>
...
export default {
  ...
  async  getCartItems() {
      ...
      this.products = await this.$store.getters["cart/getCart"];
      this.getCartTotal();
      
    },

    async getCartTotal(){
      this.total = await this.$store.getters["cart/getTotal"];
    },


   async removeCart(product_id) {
     await  this.$store.dispatch("cart/removeCart", product_id);
     await this.getCartItems();
    },


   async cartChange(product, type) {
